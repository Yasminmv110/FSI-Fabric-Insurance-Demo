...
7. [Reference Data Upload](#7-reference-data-upload)
8. [Workspace Components](#8-workspace-components)
9. [Notebooks](#9-notebooks)
10. [Pipeline Orchestration](#10-pipeline-orchestration)
11. [Lakehouse and Warehouse Layers](#11-lakehouse-and-warehouse-layers)
12. [Semantic Models and Report](#12-semantic-models-and-report)
13. [Ontologies and Data Agent](#13-ontologies-and-data-agent)
14. [Validation Checklist](#14-validation-checklist)
15. [Limitations and Notes](#15-limitations-and-notes)
...
- A Microsoft Fabric tenant with workspace creation enabled.
- Access to a Fabric capacity for the target workspace.
- Contributor or Admin access to the target Fabric workspace.
- Git integration enabled in Fabric.
- GitHub credentials configured in Fabric for this repository if using Git sync.
- Required preview features enabled as needed for Ontology and Data Agent.
- A GitHub token stored in Azure Key Vault if running `upload_reference_data_to_lakehouse` directly against this private GitHub repository archive.
...
Run the solution in this order.

| Step | Action | Fabric item |
|---:|---|---|
| 1 | Create or select the target workspace and deploy all source-controlled Fabric items | `provision_fabric_workspace_and_items` |
| 2 | Upload `Reference_Data` into `LH_Insurance` Files and stage Bronze folders | `upload_reference_data_to_lakehouse` |
| 3 | Transform structured Bronze CSVs into Silver Delta tables | `insurance_bronze_to_silver_notebook` |
| 4 | Load Silver data into Gold KPI tables | `gold.usp_Load_Silver_to_Gold` |
| 5 | Review semantic models, report, ontology, and Data Agent assets | Fabric workspace items |

### Recommended deployment path

1. Open `Notebooks/provision_fabric_workspace_and_items.Notebook/notebook-content.py` in an existing Fabric workspace.
2. Set `CAPACITY_ID`.
3. Set `RUN_GIT_SYNC = True`.
4. Fill in the Git provider settings and `GIT_CONNECTION_ID`.
5. Run all cells.
6. Confirm that the target workspace contains all Fabric items.
7. Run `upload_reference_data_to_lakehouse` or run the `Pl_Insurance_Medallion` pipeline.

### REST fallback

`provision_fabric_workspace_and_items` also includes `RUN_REST_ITEM_DEPLOYMENT` for direct item creation/update through Fabric REST APIs. Git sync is preferred because it is the most complete path for source-controlled Fabric definitions.

---

## 7. Reference Data Upload
...
## 11. Lakehouse and Warehouse Layers
...
5. Structured CSVs exist in `Files/bronze_structured_data`.
...
- If using `github_zip` mode for data upload, store GitHub access tokens in Azure Key Vault. Do not paste secrets into notebooks.
- Git sync is the preferred deployment mechanism for full Fabric item definitions.
